...
/**
 * A UDF that returns the Amplitude (maxPixelVal - minPixelVal) of a raster layer.
 * Example:
 * 		A = load 'mydata' as (raster);
 * 		B = foreach A generate AmplitudeValue(raster#'0');
 * @author Patrick Happ
 *
 */
public class AmplitudeValue extends EvalFunc<Double> {
     ...
     * @return amplitude of the layer, or null in case of processing error
     */
	@Override
	public Double exec(Tuple input) throws IOException {		
		...
		try {			
			...
			return resMax.compute(ip.iterator(), value).get() - resMin.compute(ip.iterator(), value2).get();
