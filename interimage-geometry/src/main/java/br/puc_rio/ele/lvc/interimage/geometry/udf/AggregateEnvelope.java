...
/**
 * A UDF that computes the aggregate envelope of a set of geometries. It works efficiently by computing partial envelopes and then the overall envelope.<br><br>
 * Example:<br>
 * 		A = load 'mydata' as (geom, props);<br>
 * 		B = group A by props#'class';<br>
 * 		C = foreach B generate group, AggregateEnvelope(A);
 * @author Rodrigo Ferreira
 *
 */
public class AggregateEnvelope extends EvalFunc<DataByteArray> implements Algebraic, Accumulator<DataByteArray> {

	private static final GeometryParser _geometryParser = new GeometryParser();
	
	@Override
	public DataByteArray exec(Tuple input) throws IOException {
		return new DataByteArray(new WKBWriter().write(envelope(input)));
	}
	...
	static protected Geometry envelope(Tuple input) throws ExecException {
	    ...
	    return geom_collection.getEnvelope();
	}
	
	Geometry _partialEnvelope;	
	...
	public void accumulate(Tuple b) throws IOException {
		...
		int idx = 0;
		if (_partialEnvelope != null) {
			all_geoms = new Geometry[(int)bag.size()+1];
			all_geoms[idx] = _partialEnvelope;
			idx = idx + 1;
		} else {
		...
		_partialEnvelope = new GeometryCollection(all_geoms, new GeometryFactory()).getEnvelope();
	}

	public DataByteArray getValue() {
		return new DataByteArray(new WKBWriter().write(_partialEnvelope));
	}

	public void cleanup() {
		_partialEnvelope = null;
	}
