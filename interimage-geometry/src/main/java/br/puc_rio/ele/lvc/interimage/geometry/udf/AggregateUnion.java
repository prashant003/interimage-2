 ...
 *
 */
public class AggregateUnion extends EvalFunc<DataByteArray> implements Algebraic, Accumulator<DataByteArray> {

	private static final GeometryParser _geometryParser = new GeometryParser();
	
	@Override
	public DataByteArray exec(Tuple input) throws IOException {
		return new DataByteArray(new WKBWriter().write(union(input)));
	}
	...
	static public class Intermed extends EvalFunc<Tuple> {
		@Override
		public Tuple exec(Tuple input) throws IOException {
			return TupleFactory.getInstance().newTuple(new DataByteArray(new WKBWriter().write(union(input))));
		}
	}
	...
	static protected Geometry union(Tuple input) throws ExecException {
	...
	public void accumulate(Tuple b) throws IOException {
		...
		int idx = 0;
		if (_partialUnion != null) {
			all_geoms = new Geometry[(int)bag.size()+1];
			all_geoms[idx] = _partialUnion;
			idx = idx + 1;
		} else {
		...
		_partialUnion = new GeometryCollection(all_geoms, new GeometryFactory()).buffer(0);
	}

	public DataByteArray getValue() {
		return new DataByteArray(new WKBWriter().write(_partialUnion));
	}
