 ...
 *
 */
public class AggregateCentroid extends EvalFunc<DataByteArray> implements Algebraic, Accumulator<DataByteArray> {

	private static final GeometryParser _geometryParser = new GeometryParser();
	
	@Override
	public DataByteArray exec(Tuple input) throws IOException {
		return new DataByteArray(new WKBWriter().write(centroid(input)));
	}
	...
	static public class Intermed extends EvalFunc<Tuple> {
		@Override
		public Tuple exec(Tuple input) throws IOException {
			return TupleFactory.getInstance().newTuple(new DataByteArray(new WKBWriter().write(centroid(input))));
		}
	}
	...
	static protected Geometry centroid(Tuple input) throws ExecException {
	    ...
	    return geom_collection.getCentroid();
	}
	
	Geometry _partialCentroid;	
	...
	public void accumulate(Tuple b) throws IOException {
		...
		int idx = 0;
		if (_partialCentroid != null) {
			all_geoms = new Geometry[(int)bag.size()+1];
			all_geoms[idx] = _partialCentroid;
			idx = idx + 1;
		} else {
		...
		_partialCentroid = new GeometryCollection(all_geoms, new GeometryFactory()).getCentroid();
	}

	public DataByteArray getValue() {
		return new DataByteArray(new WKBWriter().write(_partialCentroid));
	}

	public void cleanup() {
		_partialCentroid = null;
	}
